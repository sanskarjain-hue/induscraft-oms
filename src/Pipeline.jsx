...
// helper makes every place that identifies "which item is this" resilient to
// that by falling back to Mongoose's own `_id`. Used everywhere an item needs a
// stable key: React `key` props, equality checks when matching an action to the
// item it applies to, and the value passed into action handlers like advanceItem.
// New orders (post schema-fix) will have `item.id` populated and use that as
// before; old orders fall back to `item._id` and work identically.
function itemKey(item) {
  return item.id || item._id;
}

const STAGE_COUNT = STAGES.length; // 9 — last index is STAGE_COUNT - 1 ("Delivered to customer")

// FIX: previously `isDone` was `i < stageIndex`, which can never be true for the
// LAST stage in STAGES — there's no stage beyond it to push the index past. That
// meant an item that had actually reached "Delivered to customer" (the final
// stage) stayed shown as permanently "active" (amber, no checkmark) forever,
// even though there was nothing left to do. The terminal stage should count as
// done as soon as the item is AT it, not only once something moves past it.
function isStageDone(stageIdx, currentStageIndex) {
  if (stageIdx === STAGE_COUNT - 1) return currentStageIndex >= STAGE_COUNT - 1;
  return stageIdx < currentStageIndex;
}
function isStageActive(stageIdx, currentStageIndex) {
  return stageIdx === currentStageIndex && stageIdx !== STAGE_COUNT - 1;
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <Card>
        ...
        <div style={{ display: "flex", alignItems: "flex-start" }}>
          {STAGES.map((s, i) => {
            const done = isStageDone(i, maxStageIdx);
            const active = isStageActive(i, maxStageIdx);
            return (
              <div key={s} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                  ...
                  <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, background: done ? "#639922" : active ? "#FAEEDA" : "var(--color-background-primary)", border: `2px solid ${done ? "#639922" : active ? "#BA7517" : "var(--color-border-tertiary)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {done && <i className="ti ti-check" style={{ fontSize: 9, color: "white" }} />}
                  </div>
                  <div style={{ flex: 1, height: 2, background: i === STAGES.length - 1 ? "transparent" : (i < maxStageIdx ? "#639922" : "var(--color-border-tertiary)") }} />
                </div>
                <div style={{ fontSize: 9, color: active ? "#854F0B" : done ? "#3B6D11" : "var(--color-text-secondary)", textAlign: "center", marginTop: 5, maxWidth: 58, lineHeight: 1.3, fontWeight: active ? 500 : 400 }}>{s}</div>
              </div>
            );
          })}
        </div>
...
function VendorAssign({ item, order, vendors, onUpdate, onVendorCreated }) {
  ...
  function unassign() {
    const key = itemKey(item);
    const updated = { ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, vendorId: null, vendorCost: 0, committedDate: "" } : i) };
    onUpdate(updated);
  }
          ...
          <div>
            ...
            <input type="date" defaultValue={item.committedDate || ""}
              onBlur={e => {
                ...
                const key = itemKey(item);
                onUpdate({ ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, committedDate: e.target.value } : i), _editLogField: "Committed delivery date" });
              }}
          ...
          <div>
            ...
            <input type="number" defaultValue={item.vendorCost || ""}
              onBlur={e => {
                ...
                const key = itemKey(item);
                onUpdate({ ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, vendorCost: parsed } : i), _editLogField: "Vendor cost" });
              }}
              style={{ width: "100%", fontSize: 12, padding: "6px 10px", borderRadius: 8, border: "0.5px solid var(--color-border-secondary)", background: "var(--color-background-secondary)", color: "var(--color-text-primary)", fontFamily: "inherit" }} />
          </div>
        </div>
        ...
        <div key={key} style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, marginBottom: 14, overflow: "hidden" }}>
          ...
          <div style={{ padding: "14px 16px" }}>
            ...
            <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start" }}>
                {STAGES.map((s, i) => {
                  const done = isStageDone(i, item.stageIndex);
                  const active = isStageActive(i, item.stageIndex);
                  return (
                    <div key={s} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                        <div style={{ flex: 1, height: 2, background: i === 0 ? "transparent" : (i <= item.stageIndex ? "#639922" : "var(--color-border-tertiary)") }} />
                        <div style={{ width: 14, height: 14, borderRadius: "50%", flexShrink: 0, background: done ? "#639922" : active ? "#FAEEDA" : "var(--color-background-primary)", border: `1.5px solid ${done ? "#639922" : active ? "#BA7517" : "var(--color-border-tertiary)"}` }} />
                        <div style={{ flex: 1, height: 2, background: i === STAGES.length - 1 ? "transparent" : (i < item.stageIndex ? "#639922" : "var(--color-border-tertiary)") }} />
                      </div>
                      <div style={{ fontSize: 8, color: active ? "#854F0B" : done ? "#3B6D11" : "var(--color-text-secondary)", textAlign: "center", marginTop: 4, maxWidth: 52, lineHeight: 1.2 }}>{s.split(" ")[0]}</div>
                    </div>
                  );
                })}
              </div>
...
function TrackerTab({ order, role, vendors, onUpdate, currentUser }) {
  ...
  // FIX: previously matched via `i.id !== itemId`. Old orders (saved before the
  // item.id schema fix) have every item's `.id` as undefined, so that comparison
  // was `undefined !== undefined` — false for every item — meaning the per-item
  // guard never filtered anything and EVERY item in the order got advanced
  // together instead of just the one clicked. Now matches via itemKey() (id with
  // an _id fallback), and every call site below passes itemKey(item) instead of
  // the raw item.id, so this works correctly for both old and new orders.
  function advanceItem(key, extraProps = {}) {
    const updated = {
      ...order,
  ...
  function advanceSubStep(key, stepName) {
    const updated = {
      ...order,
      items: order.items.map(i => {
        if (itemKey(i) !== key) return i;
  ...
  function submitQC(key, status) {
    const notes = qcState[key]?.notes || "";
    if (status === "fail" && !notes.trim()) {
      alert("Please add a comment explaining why QC failed.");
      return;
    }
    advanceItem(key, { qcStatus: status, qcNotes: notes });
    setQcState(s => ({ ...s, [key]: { ...s[key], submitted: true } }));
  }
  ...
  return (
    <div>
      ...
      {order.items.map(item => {
        ...
        const localQC = qcState[key] || {};

        return (
          <div key={key} style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "var(--color-background-secondary)", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
              <div style={{ flex: 1 }}>
            ...
            <div style={{ padding: 16 }}>
              {STAGES.map((stageName, i) => {
                const isDone = isStageDone(i, item.stageIndex);
                const isActive = isStageActive(i, item.stageIndex);
                return (
                      ...
                      <div style={{ fontSize: 13, fontWeight: 500, color: isDone ? "#3B6D11" : isActive ? "#854F0B" : "var(--color-text-secondary)", marginBottom: 4, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                          ...
                          {isActive && userCanAdvance && stageName !== "QC" && stageName !== "Raw ready" && stageName !== "Delivered to warehouse" && (
                            <button onClick={() => advanceItem(key)} style={{ fontSize: 11, padding: "4px 12px", borderRadius: 7, border: "none", background: "#639922", color: "white", cursor: "pointer", fontFamily: "inherit", fontWeight: 500, display: "flex", alignItems: "center", gap: 4 }}>
                          ...
                          {isActive && role === "admin" && (
                            <button onClick={() => setDelayModal(key)} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 7, border: "0.5px solid #BA7517", background: "transparent", color: "#854F0B", cursor: "pointer", fontFamily: "inherit" }}>
                              Log delay
                            </button>
                          )}
                        </div>
                        ...
                        <div style={{ marginTop: 8 }}>
                          ...
                          {(role === "admin" || role === "qc") && (
                            <StagePhotoStrip
                              ...
                              canUpload={true}
                              onUpload={newPhotos => onUpdate({ ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, rawPhotos: newPhotos } : i) })}
                            />
                          )}
                          {(item.rawPhotos || []).length > 0 && !item.rawPhotosApproved && (role === "admin" || role === "sales") && (
                            <div style={{ marginTop: 8 }}>
                              ...
                              <button onClick={() => {
                                const updated = { ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, rawPhotosApproved: true } : i) };
                                onUpdate(updated);
                      ...
                      {isDone && (
                        <div>
                          ...
                          {stageName === "Delivered to customer" && (
                            <div style={{ fontSize: 12, color: "#27500A", fontWeight: 500 }}>
                              <i className="ti ti-check" style={{ fontSize: 13, marginRight: 4 }} />Delivered to customer
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      {isActive && stageName === "Finishing" && (
                        <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 6 }}>
                            ...
                            return (
                              <div key={step} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                                ...
                                {status !== "done" && (role === "admin" || role === "qc") && (
                                  <button onClick={() => advanceSubStep(key, step)} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 6, border: "0.5px solid #639922", background: "transparent", color: "#3B6D11", cursor: "pointer", fontFamily: "inherit" }}>Done</button>
                                )}
                              </div>
                            );
                          })}
                          <StagePhotoStrip
                            ...
                            onUpload={newPhotos => onUpdate({ ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, finishingPhotos: newPhotos } : i) })}
                          />
                        </div>
                      )}
                        ...
                        <div style={{ marginTop: 8 }}>
                          {!localQC.submitted ? (
                            <>
                              ...
                              <textarea
                                value={localQC.notes || ""}
                                onChange={e => setQcState(s => ({ ...s, [key]: { ...s[key], notes: e.target.value } }))}
                              ...
                              {(role === "admin" || role === "qc") && (
                                <div style={{ display: "flex", gap: 8 }}>
                                  <button onClick={() => submitQC(key, "pass")} style={{ flex: 1, fontSize: 12, padding: "8px", borderRadius: 8, border: "none", background: "#639922", color: "white", cursor: "pointer", fontFamily: "inherit", fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                                    <i className="ti ti-check" style={{ fontSize: 14 }} /> Pass QC
                                  </button>
                                  <button onClick={() => submitQC(key, "fail")} style={{ flex: 1, fontSize: 12, padding: "8px", borderRadius: 8, border: "none", background: "#C0392B", color: "white", cursor: "pointer", fontFamily: "inherit", fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                          ...
                          <StagePhotoStrip
                            ...
                            canUpload={role === "admin" || role === "qc"}
                            onUpload={newPhotos => onUpdate({ ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, qcPhotos: newPhotos } : i) })}
                          />
                        </div>
                      )}
                        ...
                        <div style={{ marginTop: 8 }}>
                          ...
                          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                            <input type="number" min="1" defaultValue={item.packetCount || ""}
                              placeholder="e.g. 3"
                              onBlur={e => {
                                const updated = { ...order, items: order.items.map(i => itemKey(i) === key ? { ...i, packetCount: parseInt(e.target.value) || 0 } : i) };
                                onUpdate(updated);
                              }}
                      ...
                      {isActive && stageName === "Delivered to warehouse" && (
                        <div style={{ marginTop: 6 }}>
                          ...
                          {(role === "admin" || role === "sales") && (
                            <button onClick={() => advanceItem(key)} style={{ fontSize: 12, padding: "6px 14px", borderRadius: 8, border: "none", background: "#639922", color: "white", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}>
